...
# Reverse lookup : qui se cache derriere un email ou un numero

## Objectif & quand

Partir d'un **email** ou d'un **numero de telephone** et retrouver le profil complet de la personne (nom, poste, entreprise, LinkedIn). A lancer pour qualifier un lead entrant (formulaire avec juste un email), identifier un appelant, ou dedupliquer un contact.
...
| Champ | Role | Exemple |
|---|---|---|
| `email` | Email a resoudre (prioritaire si rempli) | `"victor@airscale.io"` |
| `phone` | Numero a resoudre (format international) | `"+33610607076"` |
| `output_path` | Markdown de sortie | `null` |

Renseigne `email` **ou** `phone`. Si les deux sont remplis, `email` est utilise.
...
La reponse (profil enrichi, sous `body`) est rendue en fiche markdown.

> Mapping MCP : outils reverse email / reverse phone du serveur MCP Airscale.

## Livrable

Un markdown `examples/reverse-lookup.md` : fiche contact (nom, poste, entreprise, localisation, LinkedIn, et autres champs retournes).
