#!/usr/bin/env python3
"""Recherche IA structurée via Airscale /airsearch.

Pose `prompt` (+ `schema` optionnel) -> réponse structurée + sources.
Écrit un markdown.
...
def main() -> None:
    ...
    if schema_file:
        ...
        if not schema_path.is_absolute():
            # cherche relativement au dossier du skill puis à la racine
        ...
        if schema_path.exists():
            ...
            print(f"  ! schema_file introuvable ({schema_path}), recherche sans schéma", file=sys.stderr)

    print("[airsearch] /airsearch", file=sys.stderr)
    resp = post("airsearch", body)

    # Champs structurés = tout ce qui n'est pas meta
    fields = {k: v for k, v in resp.items() if k not in META}
    field_lines = "\n".join(f"- **{k}** : {v}" for k, v in fields.items()) or "- (aucun champ structuré)"
    ...
    print(f"[ok] résultat -> {path}", file=sys.stderr)
